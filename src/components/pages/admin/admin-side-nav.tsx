...
import { Card, Typography, List, Button } from '@mui/material';
import { PresentationChartBarIcon, Cog6ToothIcon, PowerIcon } from '@heroicons/react/24/solid';
import { AcademicCapIcon, UserGroupIcon, BookOpenIcon, Square3Stack3DIcon } from '@heroicons/react/24/outline';
    ...
    return (
        <Card className="h-[calc(100vh-1rem)] w-full max-w-[20rem] p-4 m-3 shadow-xl shadow-blue-gray-900/5">
            <div className="mb-2 flex-items-center-gap-4 p-4">
                <img src={APP_LOGO} alt="E-LEARNING" className="h-8 w-8" />
            ...
            <List>
                {navItems.map((item, index) => (
                    <Link to={`/admin/${item.path}`} key={index}>
                        {selected === item.label ? (
                            <Button fullWidth className="p-1 text-xs m-1" size="small" variant="outlined">
                                <li
