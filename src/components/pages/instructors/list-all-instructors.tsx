...
const ListAllInstructors: React.FC<Props> = () => {
    ...
    const [filteredInstructors, setFilteredInstructors] = useState<InstructorApiResponse[] | undefined>(undefined);
    ...
    const filteredAndSearchedInstructors = filteredInstructors?.filter(
        (instructor) =>
            filterValue.length === 0 || instructor.subjects.some((category) => filterValue.includes(category)),
    );
        ...
        <div className="h-full pb-7">
            <div className="h-1/3 p-12 flex flex-col w-full bg-skyBlueCustom items-center justify-center">
                <div className="block text-center">
                    <h1 className="p-2 text-customFontColorBlack md:text-4xl sm:text-4xl font-bold">Our Instructors</h1>
                </div>
                <div className="block text-center">
